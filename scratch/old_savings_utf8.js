  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        >
          Γé╣{item.amount} ({item.type === "Credit" ? "Cr" : "Dr"})
        </Text>
      </View>
      ...
      <View style={styles.badgeContainer}>
        <TouchableOpacity style={styles.badge} onPress={() => openCategoryModal(index)}>
          <Text style={styles.badgeText}>{item.category || "Misc"} Γû╛</Text>
        </TouchableOpacity>
        <Text style={[
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 20 }}>
        ...
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.creditText}>Γé╣{totalCredits.toFixed(2)}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Total Debits:</Text>
            <Text style={styles.debitText}>Γé╣{totalDebits.toFixed(2)}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Net Cash Flow:</Text>
            <Text style={{ ...styles.summaryValue, color: netCashFlow >= 0 ? '#27ae60' : '#e74c3c' }}>Γé╣{netCashFlow.toFixed(2)}</Text>
          </View>
          <View style={styles.summaryRow}>
          ...
          {ledgerArray.map((item, index) => (
            ...
            >
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 5 }}>
                <Text style={{ fontSize: 14, color: '#27ae60' }}>In: Γé╣{item.credit.toFixed(2)}</Text>
                <Text style={{ fontSize: 14, color: '#e74c3c' }}>Out: Γé╣{item.debit.toFixed(2)}</Text>
                <Text style={{ fontSize: 14, fontWeight: 'bold', color: item.netFlow >= 0 ? '#27ae60' : '#e74c3c' }}>Net: Γé╣{item.netFlow.toFixed(2)}</Text>
              </View>
